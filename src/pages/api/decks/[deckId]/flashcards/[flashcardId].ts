 ...
 * Retrieves a specific flashcard from a deck
 *
 * Security:
 ...
 *
 * @returns 200 OK with flashcard data on success
...
export const GET: APIRoute = async ({ params, locals }) => {
  ...
  // Step 5: Fetch flashcard and verify it belongs to the deck
  try {
    const flashcard = await getFlashcardById(supabase, flashcardId);

    // Verify flashcard belongs to the specified deck
    if (flashcard.deck_id !== deckId) {
      return new Response(
        JSON.stringify({
          error: "Fiszka nie należy do tej talii.",
        }),
        {
          status: 404,
          headers: { "Content-Type": "application/json" },
    ...
    console.error("Error pobierania fiszki:", error);

    return new Response(
      JSON.stringify({
        error: "Nie udało się pobrać fiszki",
